...
class PairEvaluation(BaseEvaluation):
    ...
    def eval_model(self, measure_file, test_sentence=None, model=None, model_path=None, polarity=False,
        ...
        # init_pair : dùng tích đề các để so với golden
        # pair : dùng predict so với golden (predict là tích ề các sau khi lọc)
        # using exact, proportional and binary measure model.
                ...
                for k in range(len(self.y_hat[index])):
                    if self.y_hat[index][k] == 1:
                        cur_predicate_tuple_pair.append(
                            self.add_polarity_to_tuple_pair(candidate_tuple_pair_col[index][k],
                                                            self.polarity_hat[index][k])
                        )
