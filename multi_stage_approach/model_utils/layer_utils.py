...
class BERTCell(nn.Module):
    def __init__(self, model_path):
        ...
        self.bert = AutoModel.from_pretrained(model_path) # load model from path
        self.hidden_size = self.bert.config.hidden_size  # 768
        self.hidden_dropout_prob = self.bert.config.hidden_dropout_prob  # 0.1

    def forward(self, input_ids, attn_mask):
        # encoded_layers shape: [layers, batch_size, tokens, hidden_size]
        # sequence_output, pooled_output, (hidden_states), (attentions)
        sequence_output, pooled_token = self.bert(input_ids, attention_mask=attn_mask)[:2]
        ...
        """
        return sequence_output, pooled_token
